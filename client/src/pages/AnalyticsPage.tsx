      ...
      <div className="fixed inset-0 bg-black/30 z-50 flex justify-center items-center p-4">
        <div className="bg-white dark:bg-zinc-900 rounded-xl shadow-lg p-6 w-full max-w-sm border border-gray-200 dark:border-zinc-800 text-center">
          ...
          <div className="flex gap-4">
            <button
                onClick={onClose}
                className="w-full bg-gray-100 dark:bg-zinc-800 hover:bg-gray-200 dark:hover:bg-zinc-700 text-gray-900 dark:text-white font-semibold py-2 rounded-lg"
            >
              Cancel
            </button>
...
const AnalyticsPage: React.FC = () => {
  ...
  const handleExportMonthlyRevenue = async () => {
    ...
    try {
      ...
      const response = await getMonthlyRevenueReport();

      const data = response.data.data;

      const headers = "month,revenue";

      const rows = data.map((item:any)=>`${item.month},${item.revenue}`);
      ...
      alert("Failed to export monthly revenue");
  ...
  const handleExportYearlyRevenue = async () => {

    try {

      setYearlyLoading(true);

      const response = await getYearlyRevenueReport();

      const data = response.data.data;

      const headers="year,revenue";

      const rows=data.map((item:any)=>`${item.year},${item.revenue}`);
      ...
      const link=document.createElement("a");
      link.href=url;
      link.download="yearly-revenue.csv";
      link.click();
      ...
      alert("Failed to export yearly revenue");
  ...
  return (
      <>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between gap-4 items-center">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <button
                  onClick={handleDownloadReport}
                  disabled={!analyticsData || summaryLoading}
                  className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-5 rounded-lg shadow-sm"
              >
                <Download size={16}/>
                Download Summary
              </button>
              ...
              <button
                  ...
                  className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-5 rounded-lg shadow-sm"
              >
                <BarChart3 size={16}/>
                {monthlyLoading ? "Processing..." : "Monthly Revenue"}
              </button>

              {/* NEW BUTTON */}

              <button
                  onClick={handleExportYearlyRevenue}
                  disabled={yearlyLoading}
                  className="flex items-center gap-2 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 px-5 rounded-lg shadow-sm"
              >
                <CalendarDays size={16}/>
                {yearlyLoading ? "Processing..." : "Yearly Revenue"}
              </button>
          ...
          {/* Rest of original dashboard unchanged */}
