...
class YearlyRevenueDataPoint(BaseModel):
    month: str
    revenue: float

class YearlyRevenueResponse(BaseModel):
    data: List[YearlyRevenueDataPoint]
...
# --- ADD THIS TO THE BOTTOM OF analytics.py ---

@router.get("/export-raw-sales")
def export_raw_sales(db: Session = Depends(get_db)):
    """
    Exports raw transactional data.
    Calculates revenue by joining the Product table to get 'selling_price'.
    """
    raw_data_query = db.query(
        models.Order.order_date.label("date"),
        models.Product.name.label("product"),
        models.OrderItem.quantity,
        # Correctly calculating revenue: Quantity * Product Selling Price
        (models.OrderItem.quantity * models.Product.selling_price).label("revenue"),
        models.Product.cost_price
    ).join(
        models.OrderItem, models.OrderItem.order_id == models.Order.id
    ).join(
        models.Product, models.Product.id == models.OrderItem.product_id
    ).all()
...
@router.get("/yearly-revenue", response_model=YearlyRevenueResponse)
def get_yearly_revenue(db: Session = Depends(get_db)):
    """
    Calculates total revenue grouped by month for the last 12 months.
    This is the core data source for the AI Yearly Review.
    """
    today = datetime.utcnow().date()
    # Go back 12 months
    ...
    # Format the data into "Month Year" format (e.g., "Jan 2025")
    formatted_data = []
    for row in yearly_query:
        month_name = month_abbr[int(row.month)]
